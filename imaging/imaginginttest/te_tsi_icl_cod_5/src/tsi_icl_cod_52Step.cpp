...
Ctsi_icl_cod_52Step::~Ctsi_icl_cod_52Step()
/**
 * Destructor
 */
	{
 ...
 */
	...
	SetTestStepName(Ktsi_icl_cod_52Step);
	}

TVerdict Ctsi_icl_cod_52Step::doTestStepPreambleL()
/**
...
TVerdict Ctsi_icl_cod_52Step::doTestStepL()
/**
 * @return - TVerdict code
 ...
 */
	...
	if (TestStepResult()==EPass)
		...
		if(decoderUid == KGIFDecoderImplementationUidValue)
			{
			// Gif tests only test 16MA destination format, so remove other display modes
			TInt index = iDisplayModes.FindL(EColor16MU);
			iDisplayModes.Remove(index);
			index = iDisplayModes.FindL(EColor16M);
			iDisplayModes.Remove(index);
			// options left as EOptionNone
			}
		else if(decoderUid == KUidIclMngPluginImplUid)
			{
...
TVerdict Ctsi_icl_cod_52Step::doTestStepPostambleL()
/**
 * @return - TVerdict code
 * Override of base class virtual
 */
